...
export function useVehicles() {
  ...
  const handleDelete = useCallback(
    async (vehicleId: number) => {
      // Confirmação do usuário com dialog customizado
      const confirmed = await confirm({
        title: t("vehiclesPage.deleteTitle"),
        message: t("vehiclesPage.deleteConfirm"),
        confirmText: t("vehiclesPage.deleteButton"),
        cancelText: t("vehiclesPage.cancelButton"),
        type: "danger",
      });
      ...
      try {
        ...
        showError(error.message || t("vehiclesPage.deleteError"));
      }
    },
    [t, showSuccess, showError, confirm]
  );
  ...
  const handleSaveVehicle = useCallback(
    async (vehicleData: Vehicle) => {
      try {
        ...
        showError(error.message || t("vehiclesPage.saveError"));
        throw error; // Re-throw para tratamento externo
      }
    },
    [selectedVehicle, loadVehicles, t, showSuccess, showError]
  );
   ...
   */
  const handleGetFromAPI = useCallback(
    async (plate: string) => {
      try {
        ...
        showError(error.message || t("vehiclesPage.apiError"));
        throw error;
      }
    },
    [t, showError]
  ...
  return {
    ...
    confirmState, // Estado do dialog de confirmação
    handleConfirm, // Handler para confirmar no dialog
    handleCancel, // Handler para cancelar no dialog
